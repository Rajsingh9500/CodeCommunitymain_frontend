    ...
    <>
      <div className="flex justify-between text-sm text-gray-300 p-3 border-b border-gray-700">
        ...
        <div className="flex items-center gap-3">
          {unread > 0 && (
            <button onClick={markAllAsRead} className="text-emerald-400 text-xs hover:underline">
              Mark all read
            </button>
          )}
...
function NotifDropdownMobile({ notifs, unread, markAllAsRead, markAsRead, deleteNotif }: any) {
  return (
    <>
      <div className="flex justify-between text-sm text-gray-300 p-3 border-b border-gray-700">
        <span>Notifications</span>
...
function UserMenu({ user, userRef, userMenu, setUserMenu, setUser, router }: any) {
  return (
    <div className="relative hidden lg:block" ref={userRef}>
      <button
        onClick={() => setUserMenu((p: boolean) => !p)}
  ...
  return (
    <>
      ...
      <aside className="fixed top-0 left-0 w-[78%] max-w-xs h-full bg-[#0d0d0e] border-r border-gray-800 p-6 z-[150] shadow-xl">
        ...
        {isReady ? (
          <>
            <div className="flex items-center gap-4 bg-gray-900/60 border border-gray-700 p-4 rounded-2xl mb-4 shadow-sm">
            ...
            <nav className="flex bg-gray-900 p-3 border border-gray-600 rounded-2xl flex-col gap-2 text-gray-300">
              ...
              <button
                ...
                className="px-4 py-3 rounded-xl border border-red-500 text-red-500 hover:bg-red-900/20 mt-2 text-sm font-semibold flex items-center gap-2"
              >
                <LogOut className="w-4 h-4" />
                Logout
              </button>
